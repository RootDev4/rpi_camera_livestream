class LiveStream {
     ...
     */
    static supportedEncodingList = ['JPEG', 'GIF', 'PNG', 'PPM', 'TGA', 'BMP']
     ...
     */
    constructor(config = { width: 1280, height: 720, fps: 16, encoding: 'JPEG', quality: 7 }) {
        this.camera = null
    ...
    setEncoding(encoding) {
        ...
        if (!supportedEncodingList.includes(encVal)) throw new Error(`[-] Encoding type '${encVal}' is not supported.`)
     ...
     * @param {*} looping Enable method looping (default: true)
     * @returns Base64 encoded image string or null
     */
    getSnapshot(looping = true) {
        return new Promise((resolve, reject) => {
            if (this.lastFrame) {
                resolve(`data:${this.mimeType};base64,${Buffer.from(this.lastFrame).toString('base64')}`)
            } else {
                if (this.verboseMode) console.log('[!] NOTICE: Taking snapshot failed because no frames have been captured yet')

                if (looping) {
                    setTimeout(() => resolve(this.getSnapshot(looping)), 500)
                } else {
                    resolve(null)
                }
            }
    ...
    /**
     * Return a stringyfied representation of supported encoding list
     * @returns Comma-separated list string
     */
    getSupportedEncodingList() {
        return LiveStream.supportedEncodingList.join(',')
    }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.camera.start(this.config, () => {
                    ...
                    this.#listen().then(ip => {
                        if (this.verboseMode) console.log(`[+] Livestream started at http://${ip}${this.pathname}`)
